...
import { GetAllCouponsResponseDTO } from '../../dtos/response/coupon-response';
...
export class GetAllCouponsUseCase {
  ...
  async execute(
    filters: GetAllCouponsFiltersDTO,
  ): Promise<GetAllCouponsResponseDTO[]> {
    const coupons = await this.couponRepository.findAll(filters);
